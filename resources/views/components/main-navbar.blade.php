<div class="h-[95px] flex items-center border-b border-black/10">
    <div class="h-[95px] flex justify-between w-full items-center">
        <a href="{{route('index')}}" class="text-[32px] font-extrabold text-black">SHOP.CO</a>
        <div class="flex gap-[14px] items-center">
            ...
            <div class="relative flex items-center">
                ...
                <form id="loginModal" action="{{route('login')}}" method="post"
                      class="hidden bg-white border shadow-xl border-black/20 absolute rounded-xl top-10 right-0 w-96 text-black space-y-5 py-[24px] px-[34px] z-50">
                    @csrf

                    @if(auth()->user() == null)
                        <h1 class="text-4xl font-bold">Login</h1>
                        <div class="bg-black rounded-xl text-white py-5 px-5 space-y-2">
                            <p class="text-center">Test User</p>
                        ...
                        @endif
                        <div class="space-y-1">
                            <label for="email" class="text-black/60 w-[80px]">Email:</label>
                            <input name="email" type="text" class="w-full bg-gray-200 py-1 rounded-xl px-2"
                                   placeholder="Input your email" value="test@example.com">
                        </div>
                        <div class="space-y-1">
                            <label for="email" class="text-black/60 w-[80px]">Password:</label>
                            <input name="password" type="password" class="w-full bg-gray-200 py-1 rounded-xl px-2"
                                   placeholder="Input your password" value="password">
                        </div>
                        <hr>
                        <button type="submit" class="bg-black text-white py-2 w-full text-center rounded-[62px]">Login
                        </button>
                    @else
                        <a href="{{route('logout')}}"
                           class="bg-black block text-white py-2 w-full text-center rounded-[62px]">Logout
                        </a>
                    @endif
                </form>
            </div>
