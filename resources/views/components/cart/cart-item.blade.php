<div class="flex justify-between">
    <div class="flex gap-[16px]">
        <img src="{{$cart->product->image}}" alt="Store Item"
             class="size-[124px] rounded-[8px] object-cover">
        <div class="flex justify-between">
            <div class="flex flex-col justify-between">
                <div class="space-y-[2px]">
                    <h1 class="text-[20px] font-bold">{{$cart->product->name}}</h1>
                    <div class="flex flex-col gap-[4px]">
                        <p>Size: <span class="text-black/60">Large</span></p>
                        <p>Color: <span class="text-black/60">White</span></p>
                    </div>
                </div>
                <h1 class="font-bold text-[24px]" id="total-price-{{$cart->id}}">
                    Rp. {{ number_format($cart->total, 0, ',', '.') }}</h1>
            </div>
        </div>
    </div>

    <form method="POST" action="{{route('update-cart', $cart->id)}}" class="flex flex-col justify-between items-end">
        @csrf
        <input type="hidden" name="product_id" value="{{$cart->product_id}}">
        <div class="flex gap-4 items-center">
            <a href="{{route('remove-cart', $cart->id)}}" class="size-[24px] flex items-center justify-center">
            ...
            <button type="submit" class="size-[24px]">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                     stroke="currentColor" class="stroke-green-600">
                    <path stroke-linecap="round" stroke-linejoin="round"
                          d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"/>
                </svg>
            </button>
        </div>
        @include('components.cart.cart-item-increase-decrease')
    </form>
</div>
